...
%%%%%%%%%%%
\setheaders{}{}
\BLOCK{for welc in welcome}
    \chapter{\VAR{welc.title}: \VAR{program_date(welc.date)}}
    \begin{flushleft}
   \section{ \VAR{welc.title}}
  \end{flushleft}
    \vspace*{0.2cm}
      \VAR{load_file(root, "welcome_receiption", welc.file)}
\BLOCK{endfor}
\newpage
...
\BLOCK{for talk in invited_talks}
  ...
  \vspace*{0.2cm}
  \VAR{load_file(root, "invited_talks", talk.file)}
  \newpage
\BLOCK{endfor}
...
\BLOCK{for award in awards}
  \begin{flushleft}
  \section{\VAR{award.title}}
  \end{flushleft}
  \textbf{\Large{\VAR{program_date(award.date)}}}\\
  \textit{\Large{\VAR{award.location}}}\\
  \Large{Session chair: \VAR{award.chair}}\\\\
  \vspace*{0.2cm}
  \VAR{load_file(root, "awards", award.file)}
  \newpage
\BLOCK{endfor}
...
%%%%%%%%%%%
\setheaders{}{}
\BLOCK{for event in social_event}
    \chapter{\VAR{event.title}: \VAR{program_date(event.date)}}
    \begin{flushleft}
   \section{ \VAR{event.title}}
  \end{flushleft}
    \vspace*{0.2cm}
      \VAR{load_file(root, "social_event", event.file)}
\BLOCK{endfor}
\newpage
...
%%%%%%%%%%%
\setheaders{}{}
\BLOCK{for local in local_guide}
    \chapter{\VAR{local.title}}
    \vspace*{0.2cm}
      \VAR{load_file(root, "local_guide", local.file)}
\BLOCK{endfor}
\newpage
...
%%%%%%%%%%%
\setheaders{}{}
\BLOCK{for venue in venue_map}
    \chapter{\VAR{venue.title}}
    \vspace*{0.2cm}
      \VAR{load_file(root, "venue_map", venue.file)}
\BLOCK{endfor}
\newpage
