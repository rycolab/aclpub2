...
\BLOCK{for chairs in program_committee.area_chairs}
  \emph{\VAR{chairs.area}}\\
  \hspace*{1em} \VAR{join(", ", chairs.members)}\\
  \newline
\BLOCK{endfor}

\textbf{Reviewers\vspace*{1em}}\\
\BLOCK{for name_block in group_by_last_name(program_committee.reviewers)}
  \VAR{join(", ", name_block)}\\
  \newline
\BLOCK{endfor}

\textbf{Secondary Reviewers\vspace*{1em}}\\
...
\begin{itemize}[leftmargin=*,label={}]
  \BLOCK{for paper in papers}
    \item \hyperlink{page.\VAR{paper.page_range[0]}}{\emph{\VAR{paper.title}}}\\ \hspace*{2em} \VAR{join(", ", paper.authors, " and ")}\dotfill \hyperlink{page.\VAR{paper.page_range[0]}}{\VAR{paper.page_range[0]}}
  \BLOCK{endfor}
\end{itemize}
\newpage
