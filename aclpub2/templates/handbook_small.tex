...
\thispagestyle{empty}
\fancyfoot[C]{}
\includepdf{/Users/uniba/Documents/GitHub/aclpub2/examples/handbook_acl2022/ACL_cover_v3.pdf}
   ...
   \BLOCK{for session in sessions}
    \BLOCK{if session.subsessions|length > 1}
      ...
      \BLOCK{for subsession in session.subsessions}
        \BLOCK{if 'Poster' in subsession.title}
         {\large \VAR{subsession.title}}\\
         \VAR{session.start_time.strftime('%H:%M')}-\VAR{session.end_time.strftime('%H:%M')} (Forum)
         \leavevmode\newline
            \noindent\rule{4cm}{0.4pt}\leavevmode\newline

        \BLOCK{endif}
        \BLOCK{if 'Poster' not in subsession.title}
...
\VAR{load_file(root, "local_guide_small", "local_guide.tex")}
